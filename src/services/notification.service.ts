...
export interface AdminNotification {
  id: string;
  type: 'account' | 'complaint_review' | 'forum_announcement' | 'complaint_update';
  icon: string;
  title: string;
  message: string;
  time: string;
...
export const notificationService = {
  async fetchAdminNotifications(): Promise<AdminNotification[]> {
    try {
      ...
      if (accountsError) {
        console.warn('Error fetching new accounts for notifications:', accountsError.message);
      }
      ...
      if (complaintsError) {
        console.warn('Error fetching complaints for notifications:', complaintsError.message);
      }
      ...
      if (newComplaints) {
        newComplaints.forEach((complaint) => {
          notifications.push({
            ...
            time: formatTimeAgo(complaint.timestamp ?? new Date().toISOString()),
      ...
      const { data: announcements, error: announcementsError } = await supabase
        .from('forum_posts')
        .select('post_id, title, created_at, is_official, account:account!acc_id(full_name)')
        .eq('is_official', true)
        .order('created_at', { ascending: false })
        .limit(25);

      if (announcementsError) {
        console.warn('Error fetching forum announcements:', announcementsError.message);
      }

      if (announcements) {
        announcements.forEach((post) => {
          const author = getFirst(post.account as { full_name?: string } | { full_name?: string }[] | null | undefined);
      ...
      const { data: complaintStatusHistory, error: statusHistoryError } = await supabase
        .from('complaint_status_history')
        .select('history_id, comp_id, from_status, to_status, changed_at, complaint:complaints!comp_id(title)')
        .order('changed_at', { ascending: false })
        .limit(25);

      if (statusHistoryError) {
        console.warn('Error fetching complaint status history:', statusHistoryError.message);
      }

      if (complaintStatusHistory) {
        complaintStatusHistory.forEach((history) => {
          if (!history.from_status || !history.to_status || history.from_status === history.to_status) {
            return;
          }

          const complaint = getFirst(history.complaint as { title?: string } | { title?: string }[] | null | undefined);
      ...
      const { data: complaintWorkUpdates, error: workUpdateError } = await supabase
        .from('complaint_work_updates')
        .select('update_id, comp_id, update_type, note, created_at, complaint:complaints!comp_id(title)')
        .order('created_at', { ascending: false })
        .limit(25);

      if (workUpdateError) {
        console.warn('Error fetching complaint work updates:', workUpdateError.message);
      }

      if (complaintWorkUpdates) {
        complaintWorkUpdates.forEach((update) => {
          const complaint = getFirst(update.complaint as { title?: string } | { title?: string }[] | null | undefined);
          const complaintTitle = complaint?.title || 'Complaint';
          const messageTail = update.note ? update.note : update.update_type === 'completion' ? 'Complaint marked as completed.' : 'Work progress was updated.';
