...
export async function findCandidateComplaints(newComplaint: any) {
  try {
    console.log(`[Duplicate Check] Finding candidates for complaint: ${newComplaint.title} (Category: ${newComplaint.category})`);
    ...
    if (error) {
      console.error('[Duplicate Check] Error finding candidate complaints from Supabase:', error);
      return [];
    }

    console.log(`[Duplicate Check] Found ${data?.length || 0} candidate(s) in Supabase.`);
    return data || [];
  } catch (err) {
...
export async function analyzeDuplicateWithGemini(newComplaint: any, candidates: any[]) {
  if (!apiKey || candidates.length === 0) {
    if (!apiKey) console.warn('[Duplicate Check] Skipping AI check: No Gemini API Key.');
    return null;
  }
...
export async function runDuplicateCheckFlow(newCompId: string, accId: string) {
  console.log(`[Duplicate Check] Starting async duplicate flow for new complaint ID: ${newCompId}`);
  try {
    ...
    if (fetchErr || !newComplaint) {
      console.error('[Duplicate Check] Failed to fetch new complaint for duplicate check:', fetchErr);
      return;
    }
    ...
    // 4. Threshold check
    if (aiResult && aiResult.is_duplicate && aiResult.score >= 80 && aiResult.matched_comp_id) {
      console.log(`[Duplicate Check] High score duplicate detected (${aiResult.score}%). Checking for existing warnings...`);
      ...
      if (!existingDup) {
        ...
        if (insertErr) {
          console.error('[Duplicate Check] Failed to insert duplicate warning into Supabase:', insertErr);
        } else {
          console.log(`[Duplicate Check] Successfully created duplicate warning in database!`);
        }
      } else {
        console.log(`[Duplicate Check] Warning already exists. Skipped insertion.`);
      }
    } else {
    ...
    console.error('[Duplicate Check] Unexpected error in runDuplicateCheckFlow:', error);
  }
}
