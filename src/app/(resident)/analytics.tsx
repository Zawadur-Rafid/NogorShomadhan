...
export default function Analytics() {
  ...
  const [data, setData] = useState<{ all: any[]; my: any[] }>({
    all: [],
    my: [],
  });
  ...
  const pending = activeComplaints.filter(
    (c) =>
      ...
      c.status.toUpperCase() === "UNVERIFIED",
  ).length;
  const inProgress = activeComplaints.filter(
  ...
  const maxCount = Math.max(
    ...(barData.datasets[0].data.length > 0 ? barData.datasets[0].data : [0]),
    1,
  );
            ...
            <View style={styles.tabContainer}>
              ...
              >
                <Text
                  style={[
                    styles.tabText,
                    filter === "MY" && styles.activeTabText,
                  ]}
                >
                  My Complaints
                </Text>
